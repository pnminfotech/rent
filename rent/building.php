...
class Employee extends Dbconfig {
    ...
    public function employeeList(){
        ...
        while( $employee = mysqli_fetch_assoc($result) ) {
          ...
          //  $empRows[] = $employee['common_meter_no'];' <button type="button" name="delete" id="'.$employee["id"].'" class="btn btn-danger btn-xs delete" ><i class="fa fa-trash"></i></button>'
            $empRows[] = ' <p class="mobile_meterno">'.$employee["common_meter_no"].'</p>';
    ...
    public function addEmployee(){
        ...
        if($isUpdated==1)
        {
            $count = count($_POST['sn1']);
            for($i=0; $i<$count; $i++)
            {
                    $insert3="INSERT INTO building_lightbill(sn,building_name,area,consumer_name,meter_no,b_id)
    		        VALUES ('$i','".$_POST["building_name"]."','".$_POST["area"]."','{$_POST['consumer_name'][$i]}','{$_POST['common_meter_no'][$i]}','$last_id')";
                    mysqli_query($this->dbConnect, $insert3);        
            }
        }
    }
    public function deleteEmployee(){
        if($_POST["empId"]) {
            ...
            if($query1==1){
                ...
                $sqlDelete1 = "
				DELETE FROM building_lightbill
		     	WHERE b_id = '".$_POST["empId"]."'";
                mysqli_query($this->dbConnect, $sqlDelete1);
            }
            if($query1==1){
                ...
                $sqlDelete1 = "
				DELETE FROM  rent_update_details
		     	WHERE b_id = '".$_POST["empId"]."'";
                mysqli_query($this->dbConnect, $sqlDelete1);
            }
        }
