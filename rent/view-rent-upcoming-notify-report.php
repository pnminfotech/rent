        ...
        <div class="row">
          <div class="col-lg-12">
            <div class="content-panel">
              <h4>Upcoming Rent Details</h4>
            <div class="adv-table" id="no-more-tables">
              <table cellpadding="0" cellspacing="0" border="0" class="table-striped  table-bordered table table-hover table-condensed cf" id="example">
...
$sql = "SELECT *,DATE_SUB(concat(YEAR(CURRENT_DATE),'-',MONTH(CURRENT_DATE),'-',rent_due_date), INTERVAL 5 DAY) as date, curdate() AS today, concat(YEAR(CURRENT_DATE),'-',MONTH(CURRENT_DATE),'-',rent_due_date) as due_date,(12 * ( YEAR(CURRENT_DATE) -YEAR(rent_paid_date) ) + (MONTH(CURRENT_DATE) - MONTH(rent_paid_date))) AS total_month FROM rent_update_details where ((DATE_SUB(concat(YEAR(CURRENT_DATE),'-',MONTH(CURRENT_DATE),'-',rent_due_date), INTERVAL 5 DAY)) <= curdate() and curdate() < (concat(YEAR(CURRENT_DATE),'-',MONTH(CURRENT_DATE),'-',rent_due_date))) and 12 * ( YEAR(CURRENT_DATE) -YEAR(rent_paid_date) ) + (MONTH(CURRENT_DATE) - MONTH(rent_paid_date)) = 1"; 
...
?>
     ...
     <tr>
                       ...
                       <td data-title="Date"> <?php echo $row["due_date"];  ?></td>
                       <td data-title="Action" ><button class="btn-xs red-border red-bg new-btn" style="Cursor:pointer" data-toggle="modal" data-target="#view-modal"  data-id="<?php echo $row['id']; ?>"  id="getUser">Pay</button>
                       <a href='view-rent-flat-pay-info.php?flat_no=<?php echo $row["flat_no"];?>&building_name=<?php echo $row["building_name"]; ?>&b_id=<?php echo $row["b_id"]; ?>&wing=<?php echo $row["wing"]; ?>'><button class="btn-warning btn-xs">View</button></a></td>	    
		</tr>
